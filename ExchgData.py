...
class ExchgData:
    known_exchanges = ['bitmex', 'bitmex-testnet', 'bfx', 'binance']
    apitrylimit = 20
    apitrysleep = 5
    tf_seconds = {
        '1m': 60,
    ...
    def fetch_candles(self, tf, start=None, end=None):
        ...
        while not rawdata and apitry < self.apitrylimit:
            try:
                ...
                self.debug("failed to connect to api to fetch candles, sleeping for %d seconds, try %d of %d" % (
            ...
            if lag > self.tf_seconds[tf]:
                ...
                if it < len(candles):
                    ...
                    self.debug("Old candles last tses now %s, %s" % (
                        utilities.ts2label(self.candles[tf][-2][0]), utilities.ts2label(self.get_last_ts(tf))))
                    trimlength = currlen
    ...
    def update_book(self):
        ...
        while not bookdata and apitry < self.apitrylimit:
            try:
                ...
                bookdata = self.exchange.fetch_order_book(self.symbol, limit=100)
            except (ccxt.ExchangeError, ccxt.DDoSProtection, ccxt.AuthenticationError, ccxt.ExchangeNotAvailable,
                    ccxt.RequestTimeout) as error:
                self.debug("failed to connect to api to fetch book, sleeping for %d seconds, try %d of %d" % (
                    self.apitrysleep + apitry * 2, apitry, self.apitrylimit))
                self.debug(error)
                time.sleep(self.apitrysleep + apitry * 2)
                apitry = apitry + 1
    ...
    def dprint_last_candles(self, tf, lookback):
        ...
        for ts in range(-lookback, 0):
            ...
            self.debug("CANDLES(%s), T: %s, O: %.2f, H: %2.f, L: %.2f, C: %.2f, V: %.2f" % (
                tf, utilities.ts2label(candle[0]), candle[1], candle[2], candle[3], candle[4], candle[5]))
